...
(function ($) {
      ...
      success: function (response) {
        if (response.success) {
          ...
          for (var i = 0; i < emailList.length; i++) {
            ...
            var run_date =
              emailList[i].run_date == null ? "" : emailList[i].run_date;
            ...
            html_content +=
              ...
              tr_class +
              '">' +
              '<div class="ste-se-td  ste-se-td-15 shouldShowWhenMinified">' +
              emailList[i].form_name +
              "</div>" +
              '<div class="ste-se-td ste-b-clr ste-se-td-15 shouldNotShowWhenMinified">' +
              status +
              "</div>" +
              '<div class="ste-se-td  ste-se-td-15 shouldShowWhenMinified">' +
              emailList[i].creation_date +
              "</div>" +
              '<div class="ste-se-td ste-b-clr  ste-se-td-15 shouldNotShowWhenMinified"><a style="cursor: pointer;" onclick="ste_get_email_data(' +
              emailList[i].form_id +
              ')">' +
              type +
              "</a></div>" +
              '<div class="ste-se-td  ste-se-td-15 shouldNotShowWhenMinified">' +
              run_date +
              "</div>" +
              '<div class="ste-se-td  ste-se-td-20 shouldNotShowWhenMinified">' +
              emailList[i].shortcode +
              "</div>" +
  ...
  document.getElementById("getdata").addEventListener("click", () => {
    ...
    var new_list_id = $(
      "#form_data_table .email_list .selected_form_list_tr"
    ).attr("id");
    /*****list_id ******/
    var list_id = $("#form_data_table .email_list .selected_form_list_tr").attr(
      "data-list-id"
    );
    if (from_name == "") {
    ...
    if (list_id == "" || list_id == undefined) {
      alert(
        "Please select the form to which you would like to broadcast your message, to do that  just click at any part of the above rows"
      );
      return false;
    }
    if (address == "") {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          $("#form_data_table .email_list .ste-se-tr.selected_form_list_tr")
            .find(".ste-se-td:eq(1)")
            .text("Running");
          $("#form_data_table .email_list .ste-se-tr.selected_form_list_tr")
            .find(".ste-se-td:eq(3)")
            .html(
              '<a style="cursor: pointer;" onclick="ste_get_email_data(' +
                list_id +
                ')">Autoresponder</a>'
            );
          var d = new Date();
          ...
          $("#form_data_table .email_list .ste-se-tr.selected_form_list_tr")
            .find(".ste-se-td:eq(4)")
            .text(output);
          $("#form_data_table .email_list .ste-se-tr").removeClass(
            "selected_form_list_tr"
          );
          $("#form_data_table .email_list .ste-se-tr ").removeClass(
            "ste_selected_tr"
          );
  ...
  $(document).on("click", "#verify_email_stedb", function (e) {
    ...
    $("#emailVerifyPopup .stedb_popup_code_field")
      .find(':input[name="code_email[]"]')
      .each(function () {
        $(this).val("");
      });
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (
          ...
          $("#send_regular_email").prop("disabled", true);
        } else if (
          response.success &&
          response.message ==
          ...
          $("#send_regular_email").prop("disabled", false);
        }
        if (response.error) {
  ...
  $("body").on(
    "click",
    "#form_data_table .email_list .ste-se-tr",
    function (event) {
      $(this)
        .addClass("selected_form_list_tr")
        .siblings()
        .removeClass("selected_form_list_tr");
      $(this)
        .addClass("ste_selected_tr")
        .siblings()
        .removeClass("ste_selected_tr");
    }
  );
  ...
  $(document).on("click", ".send_address", function (e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          $("#exampleModal .modal-body .ajax-message").html(
            '<b style="color:green;font-size: 15px;">Successfully Saved</b>'
          );
        } else {
          $("#exampleModal .modal-body .ajax-message").html(
            '<b style="color:red;font-size: 15px;">' + response.message + "</b>"
          );
        }
      },
    });
