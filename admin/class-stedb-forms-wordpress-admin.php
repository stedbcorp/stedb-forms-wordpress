...
if ( ! class_exists( 'STEDB_Forms_WordPress_Admin' ) ) {
	...
	class STEDB_Forms_WordPress_Admin {
		...
		public function __construct( $plugin_name, $version ) {
			...
			add_action( 'wp_ajax_ste_delete_form', array( $this, 'ste_delete_form' ) );
			add_action( 'ste_delete_form', array( $this, 'ste_delete_form' ) );
		...
		public function ste_delete_form() {
			global $wpdb;
			echo $_GET['id'];die;
			$user  = wp_get_current_user();
			$args  = wp_unslash( $_POST );
			$table = 'stedb_form_list';
			if ( isset( $args['nonce'] ) && wp_verify_nonce( $args['nonce'], 'ajax-nonce' ) ) {
					$form_id = sanitize_text_field( $args['form_id'] );
				if ( is_array( $form_id ) ) {
					foreach ( $form_id as $id ) {
		...
		// public function check_stedb_account_link()
		// {
		// 	alert('fjkdsnjfnd');
		// 	global $wpdb;
		// 	$args = wp_unslash( $_POST );
